...
values (1, '系统管理员', '可以管理用户角色、岗位，创建新流程', 0),
       (2, '普通用户', '仅可以发起、查看自己的审批流程', 0);
...
(
    ...
    `shift_status`        TINYINT      NOT NULL DEFAULT 0 COMMENT '当前转交处理状态（0：未转交 1：已转交待审核 2:已转交审批完毕）',
...
(
    ...
    `position_name`       VARCHAR(100) NOT NULL COMMENT '审批人岗位',
    `next_apply_approver` bigint               DEFAULT NULL COMMENT '下一个审批环节id',
    `status`              TINYINT     NOT NULL DEFAULT -1 COMMENT '状态（-1：未轮到审核 0：待审核 1：审批通过 2：已拒绝 3：转移审批给别人）',
    `comment`             VARCHAR(200)         DEFAULT NULL COMMENT '审批意见',
    `shift_flag`             TINYINT     NOT NULL DEFAULT 0 COMMENT '当前审批环节是否是转交后的审批环节（0：不是转交的环节 1：转交的审批环节）',
...
(
    ...
    `create_time`      datetime(0) NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '添加抄送用户的时间',
    PRIMARY KEY (`id`),
